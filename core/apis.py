...
from .models import ImagesForSlide, MpesaPayment, RequestForService, User,UserNotifications
...
def GetRequestWork(request):
    jobbRequest=user_serializer.RequestWorkerSerializer(data=request.data)

    

    if jobbRequest.is_valid():
        jobbRequest.save()
        return response.Response(jobbRequest.data)

    else:
        return response.Response({"message":"An error occured"})

class Req(views.APIView):
    serializer_class =user_serializer.RequestWorkerSerializer
...
class GetRequestForService(generics.ListAPIView):
    queryset=RequestForService.objects.all()
    serializer_class=user_serializer.GetRequestSerializer

    def get(self, request, *args, **kwargs):
        print( kwargs['requested_by'])
        GetRequestForService.queryset=RequestForService.objects.filter(Q(requested_by=kwargs['requested_by']))
